...
public class StoredRealtorCDataTocouchabse extends Thread {
...
new StoredRealtorDataTocouchabse("", "", "", "").start();
	...
	String leadId;
	String referralid;
	public StoredRealtorCDataTocouchabse(String contactId, String email, String ip,String leadId) {
	...
	public StoredRealtorCDataTocouchabse(String contactId, String email, String ip,String leadId,String referralid) {
	...
	public void run() {
		...
		email_from=nameEmailList.get(1);
			...
			logger.error("Error occured while couchbase connection for getHRdata"+e.getMessage());
		}
		if (jsonArray != null) {
			...
			try {
				while (test) {
					...
					try {
						...
						logger.error("Error occred While updating JSONobjects"+e.getMessage());
					}
	...
	public  JSONArray storeRealtorData() {
		...
		try {
			...
			String ur = "https://api.wistia.com/v1/stats/events.json?api_password=6bca8b64faaf09189e8266ed51b726292fd133063ffc1ca0d9fab9fc3661f3ab&start_date="
					...
					+ "&end_date="
					+ date.format(cal.getTime()) + "media_id=t3n725thcd";
			...
			logger.error("Error occured while storeRealtorData"+e.getMessage());
