...
public class ClientSurvey {
	...
	//-----------------------------------client survey-----------------------------------
		...
		public void checkClientSurvey(JSONObject jsonObject) {
			...
			logger.debug("inside check client survey------------------------>");
			...
			try {
				...
				if (list.size() == 0) {
					try {
						...
						logger.error("Error occured in checkclientSurvey"+e.getMessage());	
					}
				...
				try {
					...
					logger.error("Error occured in applicantlist of checkclientSurvey   "+e.getMessage());	
				}
				...
				logger.error("Error occured in JSON object "+e.getMessage());	
			}
