...
public class SM {
	...
	public static Double getDuration() throws Exception{
		...
		try{
			...
			TrustCertsUtil.disableSslVerification();
			//String  accessToken= Token.getInstance().getAccess_token();
			URL ringOutURL = new URL(RingCentralConstants.SMS_URL);
...
try{
	String date=time[0]+" "+time[1].substring(0,time[1].length()-5);
			...
			String phone=JsonPath.read(sb.toString(),"$.records[0].to.phoneNumber");
			String name=JsonPath.read(sb.toString(),"$.records[0].from.name");
		  ...
		  //	couchbase.storeDataInCouchBase("callLog_"+phone+"_"+name+"_"+startTime, jsonObject);
