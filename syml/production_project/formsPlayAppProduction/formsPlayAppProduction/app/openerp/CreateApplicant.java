...
public class CreateApplicant {
	...
	static Logger log = LoggerFactory.getLogger(CreateApplicant.class);
	...
	public void createApplicant(String applicantFirstName,
		...
		try {
			...
			log.error("Applicant Creation Exception"+e);
		}

	}

	public void createApplicantAddress(String applicantId, String name,
			String city, String province, String postalcode, Date date) {
		openERPSession = gHelperClass.getOdooConnection();
		try {

			log.debug("inside try");
			// //openERPSession.startSession();
			ObjectAdapter applicantAd1 = openERPSession
					.getObjectAdapter("applicant.address");
			Row applicantRow = applicantAd1.getNewRow(new String[] { "name",
					"street", "city", "province", "postal_code", "date",
					"applicant_id" });
	...
	public void createApplicantMortgageRental(String applicant_id, String name,
			int propertycounter, String selling, String rental) {
		openERPSession = gHelperClass.getOdooConnection();
		try {
			log.debug("inside create Applicant mortgage method");
			// //openERPSession.startSession();

			ObjectAdapter propertyAd3 = openERPSession
					.getObjectAdapter("applicant.mortgage");
			...
			if (selling.equalsIgnoreCase("yes")) {
				...
				if (propertycounter == 0) {
					...
					int propertyCounterLocal = GenericHelperClass
							.getPropertyCounter();
					income.setPropertyId(Integer.toString(propertyCounterLocal));
					log.debug("--- setting income property id inside createApplicantMortgageRental--- "
							+ income.getPropertyId());

					Row ad1 = propertyAd3.getNewRow(new String[] { "name",
							"monthly_rent", "selling", "property_id",
							"applicant_id" });
					ad1.put("name", name);
	...
	public void createApplicantMortgage(String applicant_id, String name,
			int property_id, String selling) {
		openERPSession = gHelperClass.getOdooConnection();
		try {
			log.debug("inside create Applicant mortgage method");
			// //openERPSession.startSession();
			...
			if (selling.equalsIgnoreCase("on")) {
				...
				if (property_id == 0) {
					...
					int propertyCounterLocal = GenericHelperClass
							.getPropertyCounter();
					Row ad1 = propertyAd3.getNewRow(new String[] { "name",
							"selling", "property_id", "applicant_id" });
					ad1.put("name", name);
					ad1.put("selling", sellingBool);
	...
	public void createApplicantProperties(String applicant_id, String name,
			String mo_condo_fees, int property_id, String selling) {
		openERPSession = gHelperClass.getOdooConnection();
		try {
			log.debug("inside create Applicant Properties method");
			// //openERPSession.startSession();
			ObjectAdapter propertyAd2 = openERPSession
					.getObjectAdapter("applicant.property");
			...
			if (selling.equalsIgnoreCase("on")) {
				...
				if (property_id == 0) {
					...
					if (mo_condo_fees.equalsIgnoreCase("-1")) {
						...
						Row ad = propertyAd2.getNewRow(new String[] { "name",
								"mo_condo_fees", "property_id", "selling",
								"applicant_id" });
	...
	public void updateApplicant(String applicantID, String mobilePhone,
			String workPhone, String homePhone, String insurance,
			String relationshipStatus, boolean nonResident, Date date) {
		...
		try {
			...
			RowCollection partners = applicantAd.searchAndReadObject(
					applicantfilters,
					new String[] { "cell", "work", "home", "sin",
							"relationship_status", "non_resident", "dob" });
			...
			applicantRow.put("non_resident", false);
			applicantRow.put("dob", date);

			// id=applicantRow.getID();
			// Tell writeObject to only write changes ie the name isn't updated
	...
	public static void main(String[] args) {
		// new CreateApplicant().assignOppertunity(340, 1, 1, 1, 1, 1, 1, 1,
		// 268);
		// new CreateApplicant().assignOppertunityPreApproval(340, 268,
		// "1",200000+"", 2+"", 1+"", 0+"", 2+"", 7+"", 0+"",5+"", 800000+"");
//		new CreateApplicant().createIncomeApplicant("2753",1,"Mob","Manager",12,true);
//		new CreateApplicant().createApplicantMortgage("2753", "strathcona",	1, "on",100);
		new CreateApplicant().updateApplicant("2753", "7878787878",
				"7612345623", "1234567890", "1232112321",
				"Married", true, new Date());
				
	}

	public void updateApplicantIfNotCanad(String applicantID,
			String mobilePhone, String relationshipStatus, Date date,
			boolean nonResident) {
		openERPSession = gHelperClass.getOdooConnection();
		try {
			...
			applicantfilters.add("id", "=", applicantID);
			RowCollection partners = applicantAd.searchAndReadObject(
					applicantfilters, new String[] { "cell", "dob",
							"relationship_status", "non_resident" });

			Row applicantRow = partners.get(0);
			applicantRow.put("dob", date);
			applicantRow.put("cell", mobilePhone);
			applicantRow.put("non_resident", true);
	...
	public void createIncomeApplicant(String applicantID, int income_type,
			String business, String position, int month, boolean supplementary) {
		openERPSession = gHelperClass.getOdooConnection();
		try {
			...
			ObjectAdapter applicantAd2 = openERPSession
					.getObjectAdapter("income.employer");

			Row ad = applicantAd2.getNewRow(new String[] { "income_type",
					"business", "position", "industry", "annual_income",
					"historical", "month", "supplementary", "property_id",
					"applicant_id" });
			log.debug("serching Applicant row fileds");
			// int propertyCounterLocal =
			// GenericHelperClass.getPropertyCounter();
	...
	public void createIncomeApplicantRental(String applicantID,
			int income_type, String business, String position,
			String annualIncome, boolean historical, boolean supplementary,
			int propertycounter) {
		openERPSession = gHelperClass.getOdooConnection();
		try {
			...
			log.debug("propertyCounterLocal inside createIncomeApplicantRental : "
					+ propertyCounterLocal);
			ad.put("income_type", income_type);
			ad.put("business", business);
			ad.put("position", position);
	...
	public void createAssetApplicant(String applicantId, String asset_Type,
		...
		try {
			...
			// openERPSession.startSession();
			ObjectAdapter applicantAd2 = openERPSession
					.getObjectAdapter("crm.asset");

			Row ad = applicantAd2.getNewRow(new String[] { "asset_type",
					"name", "value", "opportunity_id" });
			float f = 10.0f;
	...
	public void createApplicantTOLead(String applicantId, String email,
			String name) {
		openERPSession = gHelperClass.getOdooConnection();
		try {
	...
	public void assignOppertunity(int applicantId, int jobFiveYear,
			int incomeDcreased, int futureFamily, int buyNewVehicale,
			int lifeStyleChnage, int financialRiskLetter,
			int properrtyLessThanFiveyear, int leadId) {
		openERPSession = gHelperClass.getOdooConnection();
		...
		try {
			...
			com.debortoliwines.openerp.api.RowCollection partners1 = applicantAd2
					.searchAndReadObject(filters1, new String[] {
							"applicant_name", "id", "opp_rec_ids" });

			com.debortoliwines.openerp.api.Row TestRow = partners1.get(0);
			TestRow.put("id", applicantId);
			TestRow.putMany2ManyValue("opp_rec_ids", new Object[] { leadId },
					false);
			...
			com.debortoliwines.openerp.api.RowCollection partners11 = applicantAd3
					.searchAndReadObject(filters11, new String[] {
							"job_5_years", "income_decreased_worried",
							"future_family", "buy_new_vehicle",
							"lifestyle_change", "financial_risk_taker",
							"property_less_then_5_years", "id", "stage_id",
							"type" });

			for (Row r : partners11) {
				if (jobFiveYear != 0) {
					log.debug("inside for loop" + jobFiveYear + "  "
							+ incomeDcreased + "  " + futureFamily + "  "
							+ buyNewVehicale + " " + lifeStyleChnage + " "
							+ financialRiskLetter + ""
							+ properrtyLessThanFiveyear);
					...
					r.put("property_less_then_5_years",
							properrtyLessThanFiveyear);
				}
				// r.put("stage_id", 15);

				r.put("stage_id", prop.getProperty("opprtunitySatgeid"));
	...
	public void assignOppertunity(int applicantId, int userValues) {
		...
		try {
			...
			RowCollection partners = applicantAd2.searchAndReadObject(filters,
					new String[] { "applicant_name", "applicant_last_name",
							"email_personal", "opp_rec_ids" });
			...
			for (Row row : partners) {
				...
				if (opp_rec_ids != null) {
					...
					RowCollection partners1 = applicantAd3.readObject(
							opp_rec_ids, new String[] { "name", "email_from" });
					for (Row r : partners1) {
					...
					RowCollection partners2 = applicantAd3.searchAndReadObject(
							filterlead, new String[] { "name", "partner_id ",
									"email_from" });
					...
					for (Row ro : partners2) {
						...
						log.debug("name in crmlead : " + leadname
								+ " partnerid : " + partnerid + " emaillead : "
								+ emaillead);

					}

					log.debug("write into applicant");
					Row newopps = applicantAd2.getNewRow(new String[] { "name",
							"email_from" });
					...
					obj[0] = "4088";
					RowCollection partners3 = applicantAd2.searchAndReadObject(
							filters, new String[] { "applicant_name",
									"applicant_last_name", "email_personal",
									"opp_rec_ids" });
					...
					com.debortoliwines.openerp.api.Row TestRow = partners1
							.get(0);
					TestRow.put("id", applicantId);
					TestRow.putMany2ManyValue("opp_rec_ids",
							new Object[] { "4135" }, false);
	...
	public void updateOpportunity(int leadId, String propertystyle,
			String propertytype, String propertyHeating, String propertyWater,
			String propertySewage, String propertygarage,
			String propertygaragesize, String yesno, String square_footage) {
		...
		try {
			...
			case "Single":
				propertygargageSizeId = 1;

				break;
			case "Double":
				propertygargageSizeId = 2;

				break;
			case "Triple":
				propertygargageSizeId = 3;

				break;
			case "quadruple":
				propertygargageSizeId = 4;
				...
				if (propertygargageId == 3) {
					com.debortoliwines.openerp.api.RowCollection partners11 = applicantAd3
							.searchAndReadObject(filters11, new String[] {
									...
									"apartment_style", "heating", "water",
									"sewage", "garage_type", "square_footage",
									"outbuildings_value", "lot_size", "id" });
					...
					com.debortoliwines.openerp.api.RowCollection partners11 = applicantAd3
							.searchAndReadObject(filters11, new String[] {
									"property_style", "property_type",
									"apartment_style", "heating", "water",
									"sewage", "garage_type", "garage_size",
									"square_footage", "outbuildings_value",
									"lot_size", "id" });
	...
	public void updateApplicantSignatureAndIp(int applicantID,
			Date consent_dateTime, String signature_ip) {
		openERPSession = gHelperClass.getOdooConnection();
		try {
			...
			applicantfilters.add("id", "=", applicantID);
			RowCollection partners = applicantAd.searchAndReadObject(
					applicantfilters, new String[] { "consent_dateTime",
							"signature_ip" });
	...
	public void assignOppertunityPreApproval(int applicantId, int leadId,
			String what_is_your_lending_goal, String down_payment_amount,
			String propertybuying11_Test, String liveDetails,
			String monthlyRentalIncome, String desiredType,
			String desired_term, String desired_amortization,
		...
		try {
			...
			log.debug("inside OpenERP AssignOppertunity");
			// openERPSession.startSession();
			log.debug("applicantId " + applicantId + "leadId :" + leadId
					+ "what_is_your_lending_goal :" + what_is_your_lending_goal
					+ "down_payment_amount:" + down_payment_amount
					+ "propertybuying11_Test:" + propertybuying11_Test
					+ "liveDetails :" + liveDetails + "monthlyRentalIncome:"
					+ monthlyRentalIncome + "desiredType :" + desiredType
			...
			com.debortoliwines.openerp.api.RowCollection partners11 = applicantAd3
					.searchAndReadObject(filters11, new String[] {
							"what_is_your_lending_goal", "property_value",
							"down_payment_amount", "looking_fora",
							"living_in_property", "monthly_rental_income",
							"desired_mortgage_type", "desired_term",
							"desired_amortization", "id",
							"monthly_rental_income", "application_date",
							"application_no", "property_value" });
	...
	public void assignOppertunityPurchase(String applicantID, int leadId,
			String leadingGoal, String address, String purchaseprice11,
			String downpayment1, String down_payment_coming_from,
			Date expected_closing_date, Date condition_of_financing_date,
			String living_in_property, String monthlyRentalIncome,
			String desired_mortgage_type, String currentMortgageTerm,
			String lookingFor) {
		openERPSession = gHelperClass.getOdooConnection();
		System.out.println("cmmingssssssssssss");
		try {

			log.debug("inside OpenERP AssignOppertunity");
			// openERPSession.startSession();
			log.debug("applicantId " + applicantID + "leadId :" + leadId
					+ "what_is_your_lending_goal :" + leadingGoal
					+ "down_payment_amount:" + purchaseprice11 + "adress:"
					+ address + "liveDetails :" + living_in_property
					+ "monthlyRentalIncome:" + monthlyRentalIncome
					+ "desiredType :" + desired_mortgage_type + "desired_term:"
					+ currentMortgageTerm + "SDate expected_closing_date:"
					+ expected_closing_date
					+ "Datecondition_of_financing_date,:"
			...
			com.debortoliwines.openerp.api.RowCollection partners11 = applicantAd3
					.searchAndReadObject(filters11, new String[] {
							"what_is_your_lending_goal", "address",
							"property_value", "down_payment_amount",
							"down_payment_coming_from", "bank_account",
							"rrsp_amount", "other_amount", "borrowed_amount",
							"sale_of_existing_amount", "gifted_amount",
							"personal_cash_amount",
							"secondary_financing_amount",
							"sweat_equity_amount", "existing_equity_amount",
							"living_in_property", "desired_mortgage_type",
							"desired_term", "desired_amortization", "id",
							"expected_closing_date",
							"condition_of_financing_date",
							"monthly_rental_income", "address", "city",
							"province", "postal_code", "application_date",
							"application_no","type","stage_id" });
			...
			for (Row r : partners11) {
				...
				String[] downSourcePayment = down_payment_coming_from
						.split("\n");
				...
				if (condition_of_financing_date != null) {
					r.put("condition_of_financing_date",
							condition_of_financing_date);
				}
				Date date = Calendar.getInstance().getTime();
				...
				r.put("application_date", date);
				log.debug("in looop11 updated");
				 r.put("stage_id", 10); //dev-10,prod=14
	...
	public void assignOppertunityRefinance(int applicantId, int leadId,
			String leadingGoal, String address, String mortageamount1,
			String living_in_property, String monthlyRentalIncome,
			String desired_mortgage_type, String currentMortgageTerm,
			String refiAdditionalAmount, String lookingFor,
			String purchaseprice11) {
		openERPSession = gHelperClass.getOdooConnection();
		...
		try {
			...
			log.debug("applicantId " + applicantId + "leadId :" + leadId
					+ "what_is_your_lending_goal :" + leadingGoal
					+ "mortageamount1:" + mortageamount1 + "adress:" + address
					+ "liveDetails :" + living_in_property
					+ "monthlyRentalIncome:" + monthlyRentalIncome
					+ "desiredType :" + desired_mortgage_type + "desired_term:"
					+ currentMortgageTerm + "desired_amortization:"
			...
			com.debortoliwines.openerp.api.RowCollection partners11 = applicantAd3
					.searchAndReadObject(filters11, new String[] {
							"what_is_your_lending_goal", "address",
							"current_mortgage_amount", "living_in_property",
							"desired_mortgage_type", "desired_term",
							"desired_amortization", "id",
							"monthly_rental_income", "address", "city",
							"province", "postal_code", "application_date",
							"application_no", "property_value","stage_id","type" });
	...
	// for mortgagePage2Pre
	public void assignOppertunityPreApproval(int applicantId, int leadId,
			String down_payment_amount, String liveDetails,
			String monthlyRentalIncome, String purchaseprice11,String province) {
		...
		try {
			...
			com.debortoliwines.openerp.api.RowCollection partners11 = applicantAd3
					.searchAndReadObject(filters11, new String[] {
							"what_is_your_lending_goal","property_value", "down_payment_amount",
							"living_in_property", "monthly_rental_income",
							"id", "monthly_rental_income", "application_date",
							"application_no", "property_value", "province","stage_id","type"});
			...
			r.put("what_is_your_lending_goal", 1);
	...
	// for mortgagePage2Ref
	public void assignOppertunityRefinance(int applicantId, int leadId,
			String leadingGoal, String address, String living_in_property,
			String monthlyRentalIncome, String refiAdditionalAmount,
			String purchaseprice11) {
		...
		try {
			...
			log.debug("startSession");
			log.debug("applicantId " + applicantId + "leadId :" + leadId
					+ "what_is_your_lending_goal :" + leadingGoal + "adress:"
					+ address + "liveDetails :" + living_in_property
					+ "monthlyRentalIncome:" + monthlyRentalIncome
					+ "desired_term:" + refiAdditionalAmount);
			...
			log.debug("error in inserting couch base " + e);
		}
	}
	...
	public void updateOpportunity(int leadId, String propertystyle,
			String propertytype, String propertyHeating, String propertyWater,
			String propertySewage, String propertygarage,
			String propertygaragesize, String square_footage) {
		...
		try {
			...
			log.debug("propertygaragesize ***********   "+propertygaragesize);
			switch (propertygaragesize) {
			case "Single":
				propertygargageSizeId = 1;
			...
			try {
				...
				log.error("Error in updating couchbase"+e);
	...
	//for mortgagePage5a
	public void assignOppertunity(int applicantId,
	int incomeDcreased, int futureFamily, int buyNewVehicale,
	int lifeStyleChnage, int financialRiskLetter,
	int leadId){
	...
	public void updateApplicant(String applicantID, String mobilePhone,
			String workPhone, String homePhone, String insurance,
			String relationshipStatus, boolean nonResident) {
		...
		try {
			...
			log.info("Applicant updating Exception"+e);
		}

	
	}
	/*public void createApplicantMortgage(String applicant_id, String name,
			int property_id, String selling,int monthly_rent) {
		...
		try {
			...
			if (selling.equalsIgnoreCase("on")) {
				...
				if (property_id == 0) {
					...
					int propertyCounterLocal = GenericHelperClass
							.getPropertyCounter();
					Row ad1 = propertyAd3.getNewRow(new String[] { "name",
							"selling", "property_id", "applicant_id" , "monthly_rent" });
					ad1.put("name", name);
					ad1.put("selling", sellingBool);
					ad1.put("property_id", propertyCounterLocal);
					ad1.put("applicant_id", applicant_id);
					ad1.put("monthly_rent",monthly_rent);
			...
			log.info("Applicant Creation Exception for Mortgage");
			log.error("Error in createApplicantMortgage "+e);
		}
	}*/
	public void assignOppertunityPurchase(String applicantID, int leadId,
			String leadingGoal, String address, String purchaseprice11,
			String down_payment_coming_from,
			String living_in_property, String monthlyRentalIncome) {
	...
	public void createApplicantMortgage(String applicant_id, String name,
			int property_id, boolean selling) {
		...
		try {
			...
			if (selling) {
				...
				log.debug("sellingBoll value : " + selling);
	...
	public void createApplicantProperties(String applicant_id, String name,
			String mo_condo_fees, int property_id, boolean selling){
		openERPSession = gHelperClass.getOdooConnection();
		try {
			log.debug("inside create Applicant Properties method");
			// //openERPSession.startSession();
			...
			if (selling) {
				log.debug("sellingBool value : " + selling);
