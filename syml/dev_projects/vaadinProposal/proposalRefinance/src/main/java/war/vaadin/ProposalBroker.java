...
import war.couchbase.BrokerService;
...
public class ProposalBroker extends CssLayout {
	...
	public ProposalBroker(String oppid) {
		...
		expirydateinString = expirydate.getExpiryDate(oppid);
		dateformat = df.format(expirydateinString);
	...
	public void setLable() {
		...
		logger.debug("substring aa >>> " + instvalue + " " + dateformat);

		labelinstruction.setValue(instvalue + " " + dateformat);
	...
	public void SetApplicantNames(UwAppAllAlgo algoNames) {
		...
		layout.setComponentAlignment(labelCity, Alignment.MIDDLE_CENTER);
		addComponent(layout);
	...
	public void getFormLayoutforOriginal(UwAppAllAlgo algo) {
	...
	public void getFormLayoutforRecommendationSingle(UwAppAllAlgo algo) {
		...
		Button saveButton = new Button("Save", new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				brokerService.setRecommendationDetailsStyleNamewithBorderSingle(singleRecommendationFform, "borderless");
				fieldGroup.setReadOnly(true);
				try {
					fieldGroup.commit();
					...
					@SuppressWarnings("unchecked")
					RecommendDetails recdetails = ((BeanItem<RecommendDetails>) fieldGroup.getItemDataSource()).getBean();
					recdetails.toString();
	...
	public void getFormLayoutforRecommendationcombine(UwAppAllAlgo algo) {
	...
	///	binder.setReadOnly(true);
		...
		//recommendationform.addComponent(updateButton, 1, 6);
	...
	public void settableVarible(String opportunity) {
		...
		BeanItemContainer<Recommendation> recommedationContainer = new BeanItemContainer<Recommendation>(
		...
		for (Recommendation variable : rec) {
			...
			if (variable.getMortgageType().equals("2")) {
				recommedationContainer.addBean(variable);
			}

		}
		recommedationContainer.addItem(rec);
	...
	public void settableForCombined(String opportunity) {
		...
		BeanItemContainer<CombinedRecommendation> recommedationContainer = new BeanItemContainer<CombinedRecommendation>(
	...
	public void setLableForDebt() {
		...
		Label labeldebttext = new Label(debt[0].replace("-", ""), ContentMode.HTML);
		Label labeldebttext1 = new Label("<li>" + debt[1].replaceFirst("-", "") + "</li>", ContentMode.HTML);
	...
	public void setLabelforNotes() {
		...
		try {
			...
			optionlabeltext = new Label("&nbsp;" + "<li>" + map[0].replaceFirst("-", "") + "</li>", ContentMode.HTML);
			optionlabeltext1 = new Label("&nbsp;" + "<li>" + map[1].replaceFirst("-", "") + "</li>", ContentMode.HTML);
			optionlabeltext.setStyleName("labeltext");
		...
		try {
			...
			logger.error("getting null in notes : " + e.getMessage());
		}
		addComponent(layout);
	...
	private void deletebyidRecommendation(String id, String oppid) {
		...
		try {
			...
			logger.error("error while json processing :" + e.getMessage());
		} catch (JSONException e) {
			logger.error("error while json reading :" + e.getMessage());
		}
	}
	...
	private void deletebyidCombineRecommendation(String id, String oppid) {
		...
		try {
			...
			logger.error("error while getting from couchbase :" + e.getMessage());
		...
		try {
			...
			logger.error("error null CombinedRecommendation", e.getMessage());
		}
		...
		JSONObject js = null;
		try {
			js = new JSONObject(ob.writeValueAsString(Uw));
		} catch (JsonProcessingException | JSONException e) {
			logger.error("json error in storing coucbase" + e.getMessage());
		}
		couchOp = new CouchBaseOperation();
		couchOp.storeDataInCouchBase("uwapps2couchbase_allProductAlgoJSON_" + oppid, js);
	...
	private void updateRecommendation(Recommendation rec, String opportunity) {
		...
		logger.debug("++ inside updateRecommendation()++");
		...
		try {
			...
			logger.error("error while null gettting from coucbase for algo" + e.getMessage());
		...
		try {
			...
			logger.error("error in json processing", e);
		} catch (JSONException e) {
	...
	private void updateCombinedRecommendation(CombinedRecommendation rec, String opportunity) {
		...
		try {
			...
			logger.error("error in getting Uwapp NullPointer  ", e);
