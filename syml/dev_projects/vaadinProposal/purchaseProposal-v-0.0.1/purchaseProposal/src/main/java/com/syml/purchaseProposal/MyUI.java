package com.syml.purchaseProposal;
...
import com.syml.purchaseProposal.expiryDate.ExpirtyDatePageCR;
import com.syml.purchaseProposal.expiryDate.ExpiryDatePage;
import com.syml.purchaseProposal.expiryDate.ExtendExpiryDate;
import com.syml.purchaseProposal.util.ExpiryDate;
import com.syml.purchaseProposal.vaadin.ErrorDisplay;
import com.syml.purchaseProposal.vaadin.ProposalBroker;
import com.syml.purchaseProposal.vaadin.ProposalClient;
import com.syml.purchaseProposal.vaadin.ProposalReferal;
import com.syml.purchaseProposal.vaadin.ThankYou;
...
public class MyUI extends UI {
	...
	@WebServlet(value = { "/purchaseProposal/*",
	...
	public void ShowClientPage(Navigator navigator, String opp_id, String remoteAddress, ExpiryDate date)
		...
		if (date.CheckDateDifferece(opp_id)) {
			...
			//NavigateToClient(opp_id,remoteAddress);
			navigator = new Navigator(this, this);

			navigator.addView("", new ProposalClient(opp_id, remoteAddress));
			navigator.addView("errorDisplay", new ErrorDisplay("error"));
			//ProposalClient referal = new ProposalClient(opp_id,remoteAddress);
			//layout.addComponent(referal);
			navigator.addView("ThankYou", new ThankYou());

		}
	}
private void NavigateToClient(String opp_id,String remoteAddress){
	navigator = new Navigator(this, this);

	try {
		navigator.addView("", new ProposalClient(opp_id, remoteAddress));
	} catch (CouchbaseDaoServiceException | ProposalException e) {
		logger.error("error in navigate To client");
	}
	//ProposalClient referal = new ProposalClient(opp_id,remoteAddress);
	//layout.addComponent(referal);
	navigator.addView("ThankYou", new ThankYou());
