	...
	public void getFormLayoutforRecommendationcombine(UwAppAllAlgo algo) throws ProposalException {
		...
		Button saveButton = new Button("Save", new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				// recommendationform.addValidator(new
				// RegexpValidator("^[0-9]+$","Amortization should be in
				// Number"));
				// recommendationform.interestRate.addValidator(new
				// RegexpValidator("[0-9]+(\\.[0-9]{1,2})$","Interest should be
				// in Number"));
